...
What are the dimensions of `prob1`? 250 x 1
What does each row represent? length of the inner list corresponding to a given country
What does the value of `n` in row 10 of the tibble represent? the size of the inner list corresponding to the tenth country in the outer list

#### B

Are there any countries with different numbers of elements in their list? Briefly justify your response.

No. Min and max are both 23, so all countries have 23 items in the inner list.
...
`map()` returns the output as a list, while `map_chr()`, `map_dbl()`, `map_int()`, `map_lgl()` return vectors of characters, doubles, integers, and booleans, respectivly.
...
countries_df = countries %>% 
  {
    tibble(
      common_name = map_chr(., pluck, "name", "common"),
      official_name = map_chr(., pluck, "name", "official"), ## replace NULL with your previous answer
      num_capitals = map(., pluck, "capital") %>% map_int(length)
    )
  }
countries_df
```
...
```{r}
missing = countries_df %>% filter(num_capitals == 0 | (num_capitals == 1 & capitals == "")) %>%
  pull("common_name")
countries_df = countries_df %>% 
  mutate(num_capitals = ifelse(common_name %in% missing, 0L, num_capitals), 
         capitals = ifelse(common_name %in% missing, "NA_character_", capitals))
```
...
```{r}
un_regions = countries_df %>%
  filter(un_member) %>%
  count(region, subregion) %>%
  arrange(region, desc(n))

print(un_regions, n = Inf, width = Inf)
```
...
```{r}
countries_df %>%
  group_by(region, num_languages) %>%
  add_count() %>%
  ggplot(aes(x = num_languages, y = n)) +
