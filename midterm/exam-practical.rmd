...
dim(friends)
friends = friends %>% filter(speaker != "Scene Directions") %>% select(-episode, -scene, -utterance)
dim(friends)
```
...
```{r}
main_characters = ranked_speakers %>% slice_head(n = 6) %>% pull(speaker)
...
friends %>% 
  mutate(main = case_when(
    speaker %in% main_characters ~ speaker,
    TRUE ~ "Other"
  )) %>%
  count(main) %>% 
  mutate(p = n / sum(n)) %>%
...
friends %>% 
  ...
  labs(title = "Number of Lines by Season of 'Friends'", x = "Speaker", y = "Number of Lines", fill = "Speaker")
```
...
```{r}
prob5_long = prob5 %>% 
  pivot_longer(!c("speaker", "season"), names_to = c("name"), values_to = c("n"))
